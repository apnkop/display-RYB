...
void draw_cli(uint16_t framebuffer[WIDTH][HEIGHT], int framenum){
...
int main (void) {
	...
	for(int x = 0; x < WIDTH; x++){ //set framebuffer black
		for(int y =0; y < HEIGHT; y++){
			framebuffer[x][y] = 0; //black
		}
	}
	...
	for (int k =0; k<240; k++) {
		framebuffer[89][k] = 0xff00;
	}
	for (int k =0; k <240; k++) {
		framebuffer[k][138] = 0xff00;
	}
	uint16_t framebuffer_drawn[WIDTH][HEIGHT];
	int b0 = 0;
	int fork_num = 0;
	int fork_state = 1;
	...
	switch (fork_num){
		case 1:
			for (int i = 0; i < 1; i++) {
				...
				for (int x = 0; x < WIDTH; x++) {
					if (x < 88 && x > 56) {
						memmove(&framebuffer[x][229], &string_1_buffer[x-56][0], HEIGHT_CHAR*sizeof(uint16_t));
					}
					if (x > 89) {
						memmove(&framebuffer[x][139], &graphbuffer[x-90][0], HEIGHT_GRAPH*sizeof(uint16_t));
					}
				}
